package com.algorithms.week02_math_bits;

/**
 * Problem 2: Single Number
 * 
 * LeetCode #136 - Easy
 * 
 * Given a non-empty array of integers nums, every element appears twice except for one. 
 * Find that single one.
 * 
 * You must implement a solution with a linear runtime complexity and use only constant extra space.
 * 
 * Constraints:
 * - 1 <= nums.length <= 3 * 10^4
 * - -3 * 10^4 <= nums[i] <= 3 * 10^4
 * - Each element in the array appears twice except for one element which appears only once.
 * 
 * Example 1:
 * Input: nums = [2,2,1]
 * Output: 1
 * 
 * Example 2:
 * Input: nums = [4,1,2,1,2]
 * Output: 4
 * 
 * Example 3:
 * Input: nums = [1]
 * Output: 1
 * 
 * Follow-up: Can you solve this using bit manipulation?
 */
public class Problem02_SingleNumber {
    
    /**
     * Find the single number that appears only once.
     * 
     * @param nums array of integers where every element appears twice except one
     * @return the single number
     */
    public int singleNumber(int[] nums) {
        // Hint: XOR has special properties - a ^ a = 0 and a ^ 0 = a
        // Hint: XOR is commutative and associative
        // Time complexity: O(n)
        // Space complexity: O(1)
        
        int result = 0;
        for (int i : nums) {
            result ^= i;
        }       
        return result;
    }
}

